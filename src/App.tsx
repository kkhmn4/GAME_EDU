...
type GameMode = 'menu' | 'story' | 'endless' | 'time_attack' | 'hardcore' | 'gameover' | 'win';
...
const playMusic = (mode: GameMode) => {
  stopMusic();
  if (audioCtx.state === 'suspended') audioCtx.resume();

  const notes = mode === 'hardcore' 
    ? [110, 123, 130, 146] // Tense minor
    : mode === 'time_attack'
    ? [220, 247, 261, 293, 329] // Fast energetic
    : [196, 220, 247, 293, 329]; // Calm pentatonic

  const tempo = mode === 'time_attack' ? 150 : (mode === 'hardcore' ? 200 : 400);
  let step = 0;
  ...
  musicInterval = setInterval(() => {
    ...
    osc.type = mode === 'hardcore' ? 'sawtooth' : 'sine';
...
export default function App() {
  ...
  const initGame = (mode: GameMode) => {
    ...
    gameState.current = {
      ...gameState.current,
      mode: mode,
      level: mode === 'hardcore' ? 3 : 1,
      score: 0,
      lives: mode === 'hardcore' ? 1 : (mode === 'time_attack' ? 999 : 5),
      items: [],
      obstacles: [],
  ...
  const startMediaPipe = () => {
    ...
    hands.onResults((results: any) => {
      ...
      const playingModes: GameMode[] = ['story', 'endless', 'time_attack', 'hardcore'];
      if (!playingModes.includes(state.mode)) {
        return;
      }
              ...
              if (item) {
                ...
                if (item.grabbedBy === null) {
                  ...
                  if (!hitBoss && distToIncin < 100) {
                    ...
                    if (item.type === 'debris') {
                      state.score += 10 * scoreMultiplier;
                      playSound('score');
                    }
                    else if (item.type === 'satellite') { 
                      state.score = Math.max(0, state.score - 20 * scoreMultiplier); 
                      state.lives -= 1; 
                      playSound('penalty');
                    }
                    else if (item.type === 'bonus_score') {
                      state.score += 50 * scoreMultiplier;
                      playSound('powerup');
                    }
                    else if (item.type === 'bonus_slow') {
                      state.effects.slowUntil = Date.now() + 5000;
                      playSound('powerup');
                    }
                    else if (item.type === 'bomb') {
                      state.score += 10 * scoreMultiplier; // Safe disposal
                      playSound('score');
                    }
                    else if (item.type === 'boss_ship') { 
      ...
      if (state.mode === 'time_attack') {
        state.timeLeft = Math.max(0, 60 - Math.floor((now - state.startTime) / 1000));
        if (state.timeLeft <= 0) {
          state.mode = 'win'; // End of time is a "win" in time attack
      ...
      if (state.lives <= 0 && state.mode !== 'gameover' && state.mode !== 'time_attack') {
        state.mode = 'gameover';
        setFinalScore(state.score);
        setUiMode('gameover');
        playSound('penalty');
      ...
      const baseSpawnRate = (state.mode === 'endless' || state.mode === 'time_attack')
        ? Math.max(400, 2000 - Math.floor(state.score / 50) * 150) 
        : (state.level === 4 ? 1200 : 2000 - state.level * 300);
      const actualSpawnRate = isSlowed ? baseSpawnRate * 2 : (state.mode === 'hardcore' ? baseSpawnRate * 0.7 : baseSpawnRate);

      // Spawn Obstacles
      const maxObs = state.mode === 'hardcore' ? 5 : (state.level + 1);
      if ((state.level >= 2 || state.mode === 'hardcore') && state.obstacles.length < maxObs && Math.random() < 0.02) {
      ...
      if (now - state.lastSpawnTime > actualSpawnRate) {
        ...
        if (state.boss.active) {
          ...
          const levelSpeedMultiplier = (state.mode === 'endless' || state.mode === 'time_attack') ? 1 + state.score / 200 : state.level;
          const speedBoost = state.mode === 'hardcore' ? 2 : 0;
          
          state.items.push({
            id: now, type, emoji,
            x: Math.random() * (state.width - 100) + 50, y: -50,
            size: isLarge ? 80 : 40,
            speed: (Math.random() * 2 + levelSpeedMultiplier + speedBoost) * (isSlowed ? 0.5 : 1),
            isGrabbed: false,
            grabbedBy: null,
            isLarge
          });
      ...
      if (state.mode === 'time_attack') {
        canvasCtx.font = 'bold 32px monospace';
        canvasCtx.fillStyle = state.timeLeft < 10 ? '#EF4444' : '#FBBF24';
        canvasCtx.fillText(`Time: ${state.timeLeft}s`, 30, 70);
      } else {
      ...
      if (state.mode === 'story') {
        ...
        canvasCtx.fillText(`Time Attack`, 30, 110);
      } else if (state.mode === 'hardcore') {
        canvasCtx.fillText(`🔥 HARDCORE 🔥`, 30, 110);
      }
          ...
          <div className="max-w-2xl bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700 w-full">
            ...
            <div className="flex flex-wrap gap-4">
              <button 
                ...
                className="flex-1 min-w-[140px] py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl transition-all transform hover:scale-105 shadow-[0_0_20px_rgba(37,99,235,0.4)]"
              >
                Cốt Truyện
              </button>
              <button 
                onClick={() => initGame('endless')}
                className="flex-1 min-w-[140px] py-4 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-xl transition-all transform hover:scale-105 shadow-[0_0_20px_rgba(147,51,234,0.4)]"
              >
                Vô Tận
              </button>
              <button 
                onClick={() => initGame('time_attack')}
                className="flex-1 min-w-[140px] py-4 bg-yellow-600 hover:bg-yellow-500 text-white font-bold rounded-xl transition-all transform hover:scale-105 shadow-[0_0_20px_rgba(202,138,4,0.4)]"
              >
                Thử Thách (60s)
              </button>
              <button 
                onClick={() => initGame('hardcore')}
                className="flex-1 min-w-[140px] py-4 bg-red-700 hover:bg-red-600 text-white font-bold rounded-xl transition-all transform hover:scale-105 shadow-[0_0_20px_rgba(185,28,28,0.4)]"
              >
                Siêu Khó (1 Mạng)
              </button>
            </div>
